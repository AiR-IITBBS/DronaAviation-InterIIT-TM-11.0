...
class PIDController:
    ...
    def __init__(self , target_position, k_values, range):
        ...
        self.max_values = [range[0][1],range[1][1],range[2][1]]
        self.min_values = [range[0][0],range[0][0],range[0][0]]
    ...
    def calculate_state(self , current_position):
        ...
        self.out_throttle = int(self.Kp[2] * self.error[2] + self.Kd[2]*(self.error[2]-self.prev_value[2]) + self.Ki[2]*self.integral_error[2])
        self.out_pitch = int(self.Kp[1] * self.error[1] + self.Kd[1]*(self.error[1]-self.prev_value[1]) + self.Ki[1]*self.integral_error[1])
        self.out_roll = int(self.Kp[0] * self.error[0] + self.Kd[0]*(self.error[0]-self.prev_value[0]) + self.Ki[0]*self.integral_error[0])
